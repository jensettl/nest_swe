...
export class BuchFileService {
    ...
    readonly #logger = getLogger(BuchFileService.name);

    constructor(service: BuchReadService, dbService: DbService) {
     ...
     *
     * @param filename ID des zugehörigen Buches, die als Dateiname verwendet wird.
    ...
    async find(
        filename: string,
    ): Promise<BuchNotExists | FindResult | InvalidContentType> {
    ...
    async #checkFilename(filename: string) {
        ...
        if (buch === undefined) {
            ...
            this.#logger.debug('#checkFilename: BuchNotExists=%o', result);
            return result;
        }

        this.#logger.debug('#checkFilename: buch vorhanden=%o', buch);
        return undefined;
    }
