...
export class AutoValidationService {
    ...
    constructor() {
        ...
        ajvErrors(this.#ajv);
        this.#ajv.addFormat('MODELLNR', {
            type: 'string',
            validate: this.#validateMODELLNR,
        });
    }
    ...
    #checkChars(chars: string[]) {
        ...
        if (chars.length === 9) {
            // Compute the MODELLNR-10 check digit
            chars.reverse();
            ...
            // Compute the MODELLNR-13 check digit
            for (let i = 0; i < chars.length; i++) {
    ...
    #validateMODELLNR: FormatValidator<string> = (subject: string) => {
        // Checks for MODELLNR-10 or MODELLNR-13 format
        ...
        const regex =
            /^(?:MODELLNR(?:-1[03])?:? )?(?=[\dX]{10}$|(?=(?:\d+[- ]){3})[- \dX]{13}$|97[89]\d{10}$|(?=(?:\d+[- ]){4})[- \d]{17}$)(?:97[89][- ]?)?\d{1,5}[- ]?\d+[- ]?\d+[- ]?[\dX]$/u; //NOSONAR
        /* eslint-enable max-len, unicorn/no-unsafe-regex, security/detect-unsafe-regex, regexp/no-super-linear-backtracking */

        if (regex.test(subject)) {
            // Remove non MODELLNR digits, then split into an array
            const chars = subject
                .replace(/[ -]|^MODELLNR(?:-1[03])?:?/gu, '')
                .split(''); // eslint-disable-line unicorn/prefer-spread
            // Remove the final MODELLNR digit from `chars`, and assign it to `last`
